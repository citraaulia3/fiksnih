...
if menu == "Neraca Saldo":
    ...
    # Siapkan DataFrame tanpa format_rupiah untuk ekspor
    df_export = pd.DataFrame(st.session_state["saldo_awal"])
    ...
    # Simpan ke Excel di memory buffer
    excel_buffer = io.BytesIO()
    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        df_export.to_excel(writer, index=False, sheet_name="Neraca Saldo")

    # Format kolom angka sebagai Rupiah di Excel
    excel_buffer.seek(0)
    wb = load_workbook(excel_buffer)
    ws = wb["Neraca Saldo"]

    # Terapkan format Rp ke kolom Debit (B) dan Kredit (C)
    for col_idx in [2, 3]:  # Kolom B dan C
        col_letter = get_column_letter(col_idx)
        for row in range(2, ws.max_row + 1):  # Mulai dari baris 2 (data)
...
if menu == "Jurnal Umum":
    ...
    # Siapkan versi DataFrame yang tidak diformat (angka asli) untuk ekspor
    df_export = pd.DataFrame(st.session_state["jurnal"])
    df_export = pd.concat([df_export, pd.DataFrame([{
        "Tanggal": "",
        "Nama Akun": "",
        "Keterangan": "Total",
    ...
    # Terapkan format Rp ke kolom Debit (D) dan Kredit (E)
    for col_idx in [4, 5]:  # Kolom D dan E
        col_letter = get_column_letter(col_idx)
        for row in range(2, ws.max_row + 1):  # Mulai dari baris 2 (data)
            cell = ws[f"{col_letter}{row}"]
            if isinstance(cell.value, (int, float)):
...
if menu == "Neraca Saldo Setelah Disesuaikan":
    ...
    # Buat Excel di memory buffer
    df_excel = pd.DataFrame(rows)
    excel_buffer = io.BytesIO()

    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        df_excel.to_excel(writer, index=False, sheet_name="Neraca Saldo Disesuaikan")
        worksheet = writer.sheets["Neraca Saldo Disesuaikan"]

        for col_idx in [2, 3]:  # kolom Debit dan Kredit (Excel col C dan D)
            col_letter = get_column_letter(col_idx)
            for row in range(2, len(df_excel) + 2):  # mulai baris 2 karena header baris 1
    ...
    st.download_button(
        label="💾 Simpan ke Excel",
        data=excel_buffer,
        file_name="neraca_saldo_setelah_disesuaikan.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
...
if menu == "Laporan Laba Rugi":
    ...
    # Buat data tanpa format rupiah (angka polos)
    data_cleaned = []
    for row in data:
        new_row = []
        for value in row:
            if isinstance(value, str) and value.startswith("Rp"):
                # Hapus "Rp", titik, lalu ubah ke int
                value = int(value.replace("Rp", "").replace(".", "").strip())
            elif value == "":
                value = ""
            new_row.append(value)
        data_cleaned.append(new_row)

    # Buat DataFrame dari data bersih
    df_excel = pd.DataFrame(data_cleaned, columns=["Keterangan", "Jumlah1", "Jumlah2", "Jumlah3"])
    excel_buffer = io.BytesIO()

    # Simpan ke Excel dan format angka sebagai "Rp" langsung di Excel
    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        df_excel.to_excel(writer, index=False, sheet_name="Laporan Laba Rugi")
        worksheet = writer.sheets["Laporan Laba Rugi"]

        # Format angka jadi Rupiah hanya di Excel
        for col_idx in [2, 3, 4]:  # Kolom Jumlah1–Jumlah3 (Excel col B, C, D)
            col_letter = get_column_letter(col_idx)
            for row in range(2, len(df_excel) + 2):  # mulai dari baris ke-2 karena header
                cell = worksheet[f"{col_letter}{row}"]
                if isinstance(cell.value, (int, float)) and cell.value != 0:
                    cell.number_format = '"Rp"#,##0'
...
if menu == "Laporan Perubahan Ekuitas":
    ...
    #Persiapan Excel: Tanpa format Rp di DataFrame
    data_cleaned = []
    for row in data:
        keterangan, jumlah = row
        if isinstance(jumlah, str) and jumlah.startswith("Rp"):
            jumlah = int(jumlah.replace("Rp", "").replace(".", "").strip())
        elif jumlah == "":
            jumlah = ""
        data_cleaned.append([keterangan, jumlah])

    df_excel = pd.DataFrame(data_cleaned, columns=["Keterangan", "Jumlah"])
    excel_buffer = io.BytesIO()

    # Simpan ke Excel dengan format Rp langsung di worksheet
    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        df_excel.to_excel(writer, index=False, sheet_name="Perubahan Ekuitas")
        worksheet = writer.sheets["Perubahan Ekuitas"]
...
if menu == "Laporan Posisi Keuangan":
    ...
    # Ekspor ke Excel dengan format Rupiah
    data_cleaned = []
    for row in rows_stafel:
        # Bersihkan hanya kolom jumlah (tanpa Rp, tanpa minus teks)
        bersih_angka = []
        for val in row[1:]:
            if isinstance(val, str):
                val = val.replace("Rp", "").replace(".", "").replace("-", "").strip()
                val = int(val) if val.isdigit() else ""
            bersih_angka.append(val)
        data_cleaned.append([row[0]] + bersih_angka)

    df_excel = pd.DataFrame(data_cleaned, columns=["Keterangan", "Jumlah1", "Jumlah2"])
    excel_buffer = io.BytesIO()

    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        df_excel.to_excel(writer, index=False, sheet_name="Posisi Keuangan")
        worksheet = writer.sheets["Posisi Keuangan"]

        for col_idx in [2, 3]:  # Kolom B dan C (Jumlah1 dan Jumlah2)
            col_letter = get_column_letter(col_idx)
